...
export default function WhatsAppButton({ productName }: WhatsAppButtonProps) {
  const handleWhatsApp = () => {
    const message = encodeURIComponent(
      `مرحباً، أنا مهتم بمنتج ${productName} من تشكيلتكم. أود معرفة المزيد من التفاصيل.`,
    )
  ...
  return (
    <button
      ...
      className="w-full px-6 py-3 md:py-4 bg-green-600 text-white rounded-lg hover:bg-green-700 active:bg-green-800 transition-colors font-semibold flex items-center justify-center gap-2 text-base md:text-lg min-h-[44px] md:min-h-[48px]"
    >
